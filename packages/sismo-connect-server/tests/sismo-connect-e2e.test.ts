  ...
  describe("Sismo Connect server", () => {
    describe("verify with statements", () => {
      ...
      it("should throw with an invalid groupId", async () => {
        ...
        invalidSismoConnectResponse.proofs[0].claims = invalidSismoConnectResponse.proofs[0]
        ...
        await expect(
          ...
          Error(
            `The proof value ${
              ...
              (sismoConnectResponse.proofs[0].claims as ClaimRequest[])[0]?.value
            }`
          )
        );
      });
      ...
      it("Should verify", async () => {
        const sismoConnectVerifiedResult = await sismoConnect.verify(sismoConnectResponse, {
          claims: [claimRequest],
          auths: authRequests,
          signature: { message: signedMessage },
          namespace: "main",
    ...
    describe("verify without claim", () => {
      it("should throw with no claimRequest, no authRequest and no signedMessage", async () => {
        ...
        await expect(
          ...
          Error(`No claim, no auth and no signed message in the proof, please provide at least one`)
        );
      });
    });
  });
