...
describe("HydraS3 Verifier test", () => {
  ...
  let expectVerifyClaimToThrow: (proof: SismoConnectProof, expectedError: string) => Promise<void>;
  ...
  beforeAll(() => {
    ...
    const snarkClaimProof = decodeProofData(
    ...
    claimProofPublicInputs = {
      ...
      commitmentMapperPubKeyX: BigNumber.from(snarkClaimProof.input[2]).toHexString(),
      commitmentMapperPubKeyY: BigNumber.from(snarkClaimProof.input[3]).toHexString(),
      ...
      sourceVerificationEnabled: BigNumber.from(snarkClaimProof.input[12]).toHexString(),
      destinationVerificationEnabled: BigNumber.from(snarkClaimProof.input[13]).toHexString(),
    };

    const snarkVaultProof = decodeProofData(
      sismoConnectSimpleClaimResponseMock.proofs[1].proofData
    );
    ...
    vaultProofPublicInputs = {
      ...
      commitmentMapperPubKeyX: BigNumber.from(snarkVaultProof.input[2]).toHexString(),
      commitmentMapperPubKeyY: BigNumber.from(snarkVaultProof.input[3]).toHexString(),
      ...
      sourceVerificationEnabled: BigNumber.from(snarkVaultProof.input[12]).toHexString(),
      destinationVerificationEnabled: BigNumber.from(snarkVaultProof.input[13]).toHexString(),
    };
    ...
    authProofPublicInputs = {
      ...
      commitmentMapperPubKeyX: BigNumber.from(snarkAuthProof.input[2]).toHexString(),
      commitmentMapperPubKeyY: BigNumber.from(snarkAuthProof.input[3]).toHexString(),
      ...
      sourceVerificationEnabled: BigNumber.from(snarkAuthProof.input[12]).toHexString(),
      destinationVerificationEnabled: BigNumber.from(snarkAuthProof.input[13]).toHexString(),
    };
    ...
    expectVerifyClaimToThrow = async (proof: SismoConnectProof, errorMessage: string) => {
      await expect(
    ...
    describe("Validate input", () => {
      it("Should throw with incorrect input destination identifier", async () => {
        ...
        await expect(
          ...
          `userId \"0x02\" mismatch with proof input destinationIdentifier 0x1002000000000000000001553049041977724928`
        );
      });
    ...
    describe("Validate input", () => {
      it("Should throw with incorrect input Vault identifier", async () => {
        ...
        await expect(
          ...
          `userId \"0x02\" mismatch with proof input vaultIdentifier 0x106e40fa5d1d2ace741f7cab6f9d55c614bf25c3febebe148a2fa09e3bbaa096`
        );
      });
    });
