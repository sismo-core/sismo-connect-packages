...
export abstract class HydraVerifier {
  ...
  constructor({
    ...
    if (commitmentMapperPubKeys) {
      ...
      if (isImpersonationMode) {
        ...
        const address =
          commitmentMapperRegistryAddress ?? GNOSIS_COMMITMENT_MAPPER_REGISTRY_ADDRESS;
  ...
  private async _matchPublicInputWithSignedMessage({
    ...
    const expectedSignedMessage = BigNumber.from(ethers.utils.keccak256(extraData)).mod(
      SNARK_FIELD
    );
  ...
  private async _matchPublicInputWithAuth({ proof }: { proof: SismoConnectProof }) {
    const input = decodeProofData(proof.proofData).input;
    const proofPublicInputs: ProofPublicInputs = {
      destinationIdentifier: input[0],
      extraData: input[1],
    ...
    if (!BigNumber.from(auth.userId).eq(proofPublicInputs.destinationIdentifier)) {
      throw new Error(
        `userId "${BigNumber.from(
          auth.userId
    ...
    if (!BigNumber.from(proofPublicInputs.destinationVerificationEnabled).eq("1")) {
  ...
  private async _matchPublicInputWithClaim({ proof, appId, namespace }: VerifyParams) {
    const input = decodeProofData(proof.proofData).input;
    const proofPublicInputs: ProofPublicInputs = {
      destinationIdentifier: input[0],
      extraData: input[1],
    ...
    // claimType
    const claimTypFromInput = BigNumber.from(proofPublicInputs.claimType).eq("1");
    ...
    // claimValue
    if (!BigNumber.from(proofPublicInputs.claimValue).eq(BigNumber.from(claim.value))) {
      throw new Error(
    ...
    // accountsTreeValue
    const groupSnapshotId = encodeAccountsTreeValue(claim.groupId, claim.groupTimestamp);
