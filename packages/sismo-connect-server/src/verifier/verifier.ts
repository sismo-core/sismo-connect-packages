...
export class SismoConnectVerifier {
  ...
  constructor({ provider, isImpersonationMode, hydraS3 }: VerifierParams) {
    this._availableRootsRegistry = AvailableRootsRegistryContractFactory.connect({
      address: hydraS3?.availableRootsRegistryAddress || GNOSIS_AVAILABLE_ROOTS_REGISTRY_ADDRESS,
      provider,
    });
