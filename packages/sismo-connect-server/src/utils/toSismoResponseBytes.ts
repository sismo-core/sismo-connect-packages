...
const formatResponseToEncode = (sismoConnectResponse: SismoConnectResponse) => {
  return {
    ...
    signedMessage:
      (sismoConnectResponse.signedMessage as `0x${string}`) ??
        ...
        claims:
          proof.claims?.map((claim) => {
            return {
              ...
              groupTimestamp:
                !proof?.claims[0]?.groupTimestamp || proof?.claims[0]?.groupTimestamp === "latest"
                  ? pad(hexlify("latest"), { size: 16, dir: "right" })
                  : toBytes(proof?.claims[0]?.groupTimestamp),
...
export const toSismoConnectResponseBytes = (sismoConnectResponse: SismoConnectResponse) => {
